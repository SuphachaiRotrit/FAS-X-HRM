...
<template lang="pug">
modal-confirm(:loading="loading" :show="showModal")  
.page-wrapper  
      ...
      v-col(cols='12' style="text-align: -webkit-center;")
        .pt-4.d-flex.justify-center
          v-btn(
            ...
            span ย้อนกลับ      
        .header.d-flex.justify-center.text-center(style="width: 80%;")
          v-card(width="100%").mt-8
            v-form  
              v-row
                v-col(cols='12' md='7').pr-0.pb-0
                  v-card.bg-primary-lighten-1(style="minHeight:550px").v-sheet
                    v-card.bg-primary-lighten-1.d-flex.mx-8.mb-8.mt-4(style="minHeight:485px")
                      v-row 
                        v-col(cols='12').align-self-center.pb-0
                          .loadingg(v-if="loading")
                            widgets-skeletonLoader(:width="100" :height="486" :type='`image, image, image, image`')  
                          .camearaa(v-else)
                ...
                v-col(cols='12' lg='5' md='5')    
                  v-btn.mb-4.bg-success(:loading='loading' @click="manageModal({}, 'confirm')").mt-4 อัพโหลด          
